...
Worker 对未配置或误投递的 outbox kind 也返回错误并保留队列重试/告警路径，不静默 ACK。
...
用户侧会话归档仍是本地索引状态更新，不调用 Sidecar；平台侧归档请求已沿用同一账号锁，
并通过 Job/Outbox 执行，不能在 API 请求中直接调用平台。真实 selector 未部署时，Worker
会将 Job 安全标记失败。
