...
当前已完成 M5 的运营概览、用户、账号、Worker/队列、Adapter、风险、审计、站点设置以及权益与卡密管理：统一 Web Admin 页面通过管理员角色保护，页面路由为 `/admin`、`/admin/users`、`/admin/accounts`、`/admin/workers`、`/admin/jobs`、`/admin/adapters`、`/admin/risks`、`/admin/settings`、`/admin/audit`、`/admin/entitlement` 及其明细路由；页面分别消费 `/api/v1/admin/overview`、`/api/v1/admin/users`、`/api/v1/admin/users/{id}`、`PATCH /api/v1/admin/users/{id}`、`/api/v1/admin/accounts`、`/api/v1/admin/workers`、`/api/v1/admin/jobs`、`/api/v1/admin/adapters`、`/api/v1/admin/risks`、`/api/v1/admin/settings`、`/api/v1/admin/audit-logs`、`/api/v1/admin/entitlement-plans`、`/api/v1/admin/card-batches`、`/api/v1/admin/redemptions` 和 `/api/v1/admin/users/{id}/entitlements` 展示 DAU、发送成功率、失败码 Top、风险账号、队列/Worker 运行态、Generic Job 生命周期、用户资源、账号状态、Capability、Adapter 健康、受约束站点配置、审计、卡密摘要和用户授权时间线；用户详情支持禁用/恢复，禁用事务性撤销全部 AuthSession/Refresh Token，恢复要求重新登录；Adapter 启停、账号暂停/恢复、用户禁用/恢复、站点配置更新、方案/批次创建与停用、人工 Grant/Revoke、未使用卡密撤销等动作写入审计日志，配置值本身不写入审计详情。Admin 用户、账号、风险、审计、权益方案/批次/兑换主列表以及卡密明细/用户授权详情已接入统一 cursor pagination，默认 50 条并由对应页面的“加载更多”读取后续页面。
Generic Job 生命周期也已接入 `/admin/jobs`，支持状态/类型筛选和统一 cursor pagination；列表只返回脱敏生命周期元数据，不开放事件 payload。
