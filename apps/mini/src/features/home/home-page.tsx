...
export function HomePage() {
  ...
  return <View className="mini-page home-page">
    ...
    <View className="account-selector home-account-card home-reveal home-reveal-2" onClick={() => setAccountPickerOpen((current) => !current)}>
      <Avatar src={account?.avatar_url} name={account?.nickname || '未绑定账号'} size="large" />
      <View className="account-selector-copy"><Text className="account-selector-label">当前账号</Text><Text className="account-selector-name">{account ? accountTabLabel(account) : '还没有绑定抖音账号'}</Text><Text className="account-selector-status"><StatusDot tone={account?.binding_status === 'bound' ? 'green' : 'amber'} />{account ? `${accountStatus(account.binding_status)} · ${sessionStatus(account.session_status)}` : '绑定后开始管理会话火花'}</Text></View>
      <View className="home-account-action"><Text className="home-account-action-label">切换</Text><Text className="chevron" aria-hidden="true">›</Text></View>
    </View>

    {accountPickerOpen && <View className="account-picker"><View className="account-picker-heading"><Text>切换账号</Text><Text className="account-picker-count">{data.accounts.length} 个账号</Text></View>{data.accounts.length === 0 ? <Text className="muted">暂未绑定账号</Text> : data.accounts.map((item) => <Button key={item.id} className={`account-picker-row ${item.id === activeAccountId ? 'account-picker-row-active' : ''}`} onClick={() => { setSelectedAccountId(item.id); setAccountPickerOpen(false) }}><Avatar src={item.avatar_url} name={accountTabLabel(item)} /><View className="account-picker-copy"><Text>{accountTabLabel(item)}</Text><Text className="muted">{accountStatus(item.binding_status)} · {sessionStatus(item.session_status)}</Text></View>{item.id === activeAccountId && <Text className="account-check">✓</Text>}</Button>)}<Button className="account-picker-add" onClick={() => Taro.switchTab({ url: '/pages/accounts/index' })}><Text className="plus">+</Text> 添加或管理账号</Button></View>}

    <View className="home-summary home-reveal home-reveal-3"><View className="summary-heading"><View><Text className="summary-eyebrow">TODAY · {formatToday()}</Text><Text className="summary-title">今日概览</Text><Text className="summary-caption">数据实时更新</Text></View><View className={`summary-health summary-health-${overallHealth.tone}`}><StatusDot tone={overallHealth.tone} /><Text>{overallHealth.label}</Text></View></View><View className="summary-grid"><SummaryMetric label="活跃账号" value={data.accounts.filter((item) => item.binding_status === 'bound').length} /><SummaryMetric label="活跃任务" value={activeTaskCount} /><SummaryMetric label="已完成" value={todayStats.successful} /><SummaryMetric label="待处理" value={todayStats.pending} /></View></View>
