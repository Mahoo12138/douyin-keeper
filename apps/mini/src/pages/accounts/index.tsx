...
function BindingSuccessScreen({ account, onDone, onViewFriends }: { account: Account | null; onDone: () => void; onViewFriends: () => void }) {
  return <View className="mini-page account-page account-binding-page"><BindingTopbar title="添加成功" onBack={onDone} /><FlowSteps current={5} /><View className="binding-success-visual"><Image className="binding-success-image" src={accountSuccess} mode="aspectFit" /></View><Text className="binding-success-title">添加成功！</Text><Text className="muted binding-success-copy">你已成功添加抖音账号</Text>{account && <View className="binding-success-account"><Avatar src={account.avatar_url} name={accountTabLabel(account)} size="large" /><View><Text className="binding-success-name">{accountTabLabel(account)}</Text><Text className="muted">账号已安全绑定</Text></View></View>}<Button className="account-primary-button" onClick={onDone}>完成</Button><Button className="binding-friends-link" onClick={onViewFriends}>去查看会话</Button></View>
}
