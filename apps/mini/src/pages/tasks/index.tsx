...
export default function Tasks() {
  ...
  function openCreate() {
    const account = accounts.find((item) => item.binding_status === 'bound' && (friendsByAccount[item.id] ?? []).some((friend) => friend.platform_identity_status === 'resolved'))
    const friend = account ? (friendsByAccount[account.id] ?? []).find((item) => item.platform_identity_status === 'resolved') : undefined
    if (!account) {
      void Taro.showModal({ title: '请先绑定抖音账号', content: '绑定账号后，才能为好友创建火花任务。' })
      return
    }
    if (!friend) {
      void Taro.showModal({ title: '暂无可用好友', content: '请先同步好友，并等待好友身份确认后再创建任务。' })
      return
    }
    setError('')
    setCreateDraft({ accountId: account.id, friendId: friend.id, enabled: true, windowStart: '19:30', windowEnd: '22:30', message: '', allowFirstMessage: false })
    setScreen('create')
  }
  ...
  async function saveTask() {
    ...
    setError('')
    try {
      const updated = await updateTask(token, selectedTask.id, { window_start: taskTimePayload(draft.windowStart), window_end: taskTimePayload(draft.windowEnd), message: { kind: selectedTask.message.kind, body: draft.message.trim() }, allow_first_message: draft.allowFirstMessage })
      setTasks((current) => current.map((item) => item.id === updated.id ? updated : item))
      setScreen('detail')
      setDraft(null)
  ...
  async function saveCreatedTask() {
    ...
    try {
      const created = await createTask(token, {
        ...
        message: { kind: 'text', body: createDraft.message.trim() },
...
function CreateTask({ draft, accounts, friendsByAccount, busy, error, onBack, onDraftChange, onSave }: { draft: CreateDraft; accounts: Account[]; friendsByAccount: Record<string, Friend[]>; busy: string; error: string; onBack: () => void; onDraftChange: (draft: CreateDraft) => void; onSave: () => void }) {
  const selectableAccounts = accounts.filter((account) => account.binding_status === 'bound' && (friendsByAccount[account.id] ?? []).some((friend) => friend.platform_identity_status === 'resolved'))
  ...
  const availableFriends = (selectedAccount ? friendsByAccount[selectedAccount.id] ?? [] : []).filter((friend) => friend.platform_identity_status === 'resolved')
