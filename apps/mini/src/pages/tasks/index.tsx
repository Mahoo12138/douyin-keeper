...
export default function Tasks() {
    ...
    const poll = async () => {
      try {
        ...
        if (job.status === 'succeeded') {
          ...
          setError(jobErrorMessage(job.error_code, '任务执行失败，请查看执行记录。'))
        }
      } catch (cause) {
        if (!active) return
        setRunJobId('')
  ...
  return <MiniPageLayout pageClassName="task-page" align="start" title={<View><Text className="task-page-kicker">Douyin Keeper</Text><Text className="task-page-title">任务</Text></View>} action={<View className="task-header-actions"><Button className="task-template-link" onClick={openTemplates}>模板</Button><MiniNavbarAction className="task-new-button" ariaLabel="新建任务" onClick={openCreate}>+</MiniNavbarAction></View>}><View className="task-filter-tabs">{([{ value: 'all', label: '全部' }, { value: 'enabled', label: '运行中' }, { value: 'paused', label: '已暂停' }] as const).map((item) => <Button key={item.value} className={filter === item.value ? 'task-filter-active' : ''} onClick={() => setFilter(item.value)}>{item.label}</Button>)}</View><View className="task-overview"><Overview label="运行中" value={tasks.filter((task) => task.enabled).length} tone="green" /><Overview label="已暂停" value={tasks.filter((task) => !task.enabled).length} tone="amber" /><Overview label="今日任务" value={tasks.length} tone="blue" /></View>{error && <View className="task-inline-error"><Text>{error}</Text></View>}{visibleTasks.length === 0 ? <EmptyTasks onCreate={openCreate} /> : <View>{visibleTasks.map((task) => <TaskCard key={task.id} task={task} friend={friends[task.friend_id]} account={accounts.find((account) => account.id === task.account_id)} busy={busy === task.id} onSelect={() => { setSelectedTaskId(task.id); setScreen('detail') }} onToggle={(enabled) => void toggleTask(task, enabled)} />)}</View>}</MiniPageLayout>
}
...
function CreateTask({ draft, accounts, friendsByAccount, templates, busy, error, onBack, onDraftChange, onSave }: { draft: CreateDraft; accounts: Account[]; friendsByAccount: Record<string, Friend[]>; templates: MessageTemplate[]; busy: string; error: string; onBack: () => void; onDraftChange: (draft: CreateDraft) => void; onSave: () => void }) {
  ...
  const accountOptions = selectableAccounts.map((account) => accountTabLabel(account))
...
function TaskHistory({ task, friend, history, onBack }: { task: Task; friend?: Friend; history: HistoryItem[]; onBack: () => void }) { return <MiniPageLayout pageClassName="task-page" title="执行记录" showBack onBack={onBack}><View className="task-history-summary"><Text className="task-section-title">{friend?.display_name || '未命名会话'}</Text><Text className="muted">{task.window_start.slice(0, 5)} ～ {task.window_end.slice(0, 5)} · 最近记录</Text><View className="history-summary-grid"><Overview label="总执行" value={history.length} tone="blue" /><Overview label="成功" value={history.filter((item) => item.status === 'succeeded').length} tone="green" /><Overview label="需关注" value={history.filter((item) => ['failed', 'retry_wait'].includes(item.status)).length} tone="amber" /></View></View>{history.length === 0 ? <View className="task-empty-small"><Text className="task-empty-title">暂无执行记录</Text><Text className="muted">任务执行后，结果会显示在这里。</Text></View> : <View className="task-history-list">{history.map((item) => <View className="task-history-row" key={item.id}><View className={`history-dot history-dot-${item.status}`} /><View className="task-history-row-copy"><Text>{formatClock(item.scheduled_at)} <Text className={`task-history-status task-history-status-${item.status}`}>{historyLabel(item.status)}</Text></Text><Text className="muted">{jobErrorMessage(item.error_code, item.intent_type === 'manual' ? '手动执行' : '定时执行')}</Text></View><Text className="muted">{formatDay(item.scheduled_at)}</Text></View>)}</View>}</MiniPageLayout> }
...
function EmptyTasks({ onCreate }: { onCreate: () => void }) { return <View className="task-empty"><MiniRemoteImage className="task-empty-image" name="tasks/task-checklist.png" mode="aspectFit" /><Text className="task-empty-title">还没有任务</Text><Text className="muted">在这里创建第一个火花任务，<br />之后即可随时启停和执行。</Text><Button className="task-primary-button" onClick={onCreate}>新建任务</Button></View> }
function GuestTasks() { return <MiniPageLayout pageClassName="task-page" title="任务" align="start"><View className="task-empty"><MiniRemoteImage className="task-empty-image" name="tasks/task-checklist.png" mode="aspectFit" /><Text className="task-empty-title">请先登录</Text><Text className="muted">登录后才能查看和管理任务。</Text><Button className="task-primary-button" onClick={() => Taro.switchTab({ url: '/pages/login/index' })}>去登录 / 绑定</Button></View></MiniPageLayout> }
function TaskError({ message, onRetry }: { message: string; onRetry: () => void }) { return <MiniPageLayout pageClassName="task-page" title="任务" align="start"><View className="task-empty"><Text className="task-error-mark">!</Text><Text className="task-empty-title">任务暂时不可用</Text><Text className="muted">{message || '请检查网络后重试。'}</Text><Button className="task-secondary-button" onClick={onRetry}>重新加载</Button></View></MiniPageLayout> }
function LoadingTasks() { return <MiniPageLayout pageClassName="task-page" title={<View className="task-skeleton task-skeleton-header" />} align="start"><View className="task-skeleton task-skeleton-tabs" /><View className="task-skeleton task-skeleton-card" /><View className="task-skeleton task-skeleton-card" /></MiniPageLayout> }
