...
import { getMe, getNotificationPreferences, listMyEntitlementGrants, listNotifications, linkWechatMini, loginWechatMini, markAllNotificationsRead, markNotificationRead, MiniApiError, myEntitlement, redeemCardCode, updateNotificationPreferences } from '@/lib/api'
...
export default function Me() {
  ...
  if (!hasToken) return <AuthGate linkCode={linkCode} busy={busy} message={message} onLinkCodeChange={setLinkCode} onLogin={() => void runWechatLogin()} onLink={() => void runLink()} />
...
function AuthGate({ linkCode, busy, message, onLinkCodeChange, onLogin, onLink }: { linkCode: string; busy: string; message: string; onLinkCodeChange: (value: string) => void; onLogin: () => void; onLink: () => void }) { return <View className="mini-page me-page me-auth-page"><Image className="auth-mascot" src={mascotSprout} mode="aspectFit" /><Text className="auth-title">我的</Text><Text className="muted auth-subtitle">登录后管理权益、通知与安全设置。</Text><View className="me-auth-card"><Text className="me-section-title">微信登录</Text><Text className="muted">已绑定过微信身份？直接登录移动控制台。</Text><Button className="me-primary-button" disabled={busy !== ''} onClick={onLogin}>{busy === 'login' ? '处理中…' : '微信登录'}</Button></View><View className="me-auth-card"><Text className="me-section-title">绑定 PC 账号</Text><Text className="muted">在 PC 端“我的”页面生成一次性绑定码。</Text><Input className="me-code-input" value={linkCode} maxlength={32} placeholder="例如 ABCD-EFGH" onInput={(event) => onLinkCodeChange(event.detail.value)} /><Button className="me-secondary-button" disabled={busy !== '' || !linkCode.trim()} onClick={onLink}>绑定已有账号</Button></View>{message && <View className="me-inline-error"><Text>{message}</Text></View>}</View> }
...
function HelpList({ sections }: { sections: { title: string; body: string }[] }) { return <View className="help-list-me">{sections.map((section) => <View className="help-row-me" key={section.title}><Text>{section.title}</Text><Text className="muted">{section.body}</Text></View>)}</View> }
