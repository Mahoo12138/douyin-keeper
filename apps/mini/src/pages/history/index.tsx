...
function HistoryCard({ item }: { item: HistoryItem }) {
  ...
  return <View className="card history-card"><View className="card-heading"><View><Text className="card-title">{item.friend.display_name}</Text><Text className="muted">{accountTabLabel(item.account)} · {formatTime(item.scheduled_at)}</Text></View><Text className={`history-status history-status-${status.tone}`}>{status.label}</Text></View><View className="history-grid"><View><Text className="history-label">任务</Text><Text className="history-value">{taskLabel(item)}</Text></View><View><Text className="history-label">通道</Text><Text className="history-value">{job?.adapter || '待分配'}</Text></View></View>{item.error_code && <View className="history-error"><Text>{jobErrorMessage(item.error_code)}</Text></View>}{item.intent_type === 'manual' && <Text className="muted history-manual">手动执行</Text>}</View>
}

function GuestHistory() { return <MiniPageLayout title="发送记录" showBack><View className="card empty-card"><Text className="card-title">请先登录</Text><Text className="muted">登录后才能查看发送记录。</Text><Button className="mini-button primary-button" onClick={() => Taro.switchTab({ url: '/pages/login/index' })}>去登录 / 绑定</Button></View></MiniPageLayout> }
function ErrorHistory({ message, onRetry }: { message: string; onRetry: () => void }) { return <MiniPageLayout title="发送记录" showBack><View className="card empty-card"><Text className="card-title">发送记录暂时不可用</Text><Text className="muted">{message || '请检查网络连接后重试。'}</Text><Button className="mini-button secondary-button" onClick={onRetry}>重新加载</Button></View></MiniPageLayout> }
function LoadingHistory() { return <MiniPageLayout title="发送记录" showBack><View className="card loading-card"><View className="loading-line loading-line-wide" /><View className="loading-line" /><View className="loading-block" /></View><View className="metric-grid"><View className="loading-block metric-loading" /><View className="loading-block metric-loading" /><View className="loading-block metric-loading" /></View></MiniPageLayout> }
