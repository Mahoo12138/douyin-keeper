...
import { enabledTaskCount, replaceFriend, replaceTask, taskForFriend } from '@/features/spark/spark-utils'
...
export default function Spark() {
  ...
  async function toggleFriend(friendId: string, enabled: boolean) {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : '好友开关更新失败')
    } finally {
      setBusyKey('')
    }
  }
...
function FriendCard({ friend, task, busyKey, onFriendToggle, onTaskToggle }: { friend: SparkData['friends'][number]; task?: SparkData['tasks'][number]; busyKey: string; onFriendToggle: (enabled: boolean) => void; onTaskToggle?: (enabled: boolean) => void }) {
  const friendBusy = busyKey === `friend:${friend.id}`
  const taskBusy = task ? busyKey === `task:${task.id}` : false
  const identityLabel = friend.platform_identity_status === 'resolved' ? '身份已确认' : friend.platform_identity_status === 'pending' ? '身份待确认' : '需处理'
  return <View className="card friend-card"><View className="card-heading"><View><Text className="card-title">{friend.nickname || friend.display_name}</Text><Text className="muted">{friend.short_id ? `抖音号 ${friend.short_id}` : identityLabel}</Text></View><Text className="streak-badge">{friend.streak_days} 天火花</Text></View><Text className="muted">{friend.has_conversation ? '已有会话' : '暂无会话'} · {friend.last_sent_at ? `上次发送 ${formatDate(friend.last_sent_at)}` : '还未发送'}</Text><View className="toggle-row"><View><Text className="toggle-title">火花维护</Text><Text className="muted">{friend.spark_enabled ? '每日保持火花' : '暂不维护'}</Text></View><Switch checked={friend.spark_enabled} disabled={friendBusy} color="#1f5bd8" onChange={(event) => onFriendToggle(event.detail.value)} /></View>{task ? <View className="task-panel"><View className="toggle-row"><View><Text className="toggle-title">每日任务</Text><Text className="muted">{task.window_start.slice(0, 5)}–{task.window_end.slice(0, 5)} · {task.timezone}</Text></View><Switch checked={task.enabled} disabled={taskBusy} color="#15966b" onChange={(event) => onTaskToggle?.(event.detail.value)} /></View><Text className="task-message">{task.message.body || (task.message.kind === 'sticker' ? '贴纸消息' : '未填写消息')}</Text></View> : <View className="task-panel"><Text className="muted">尚未配置每日任务，请在 PC 端创建。</Text></View>}</View>
}

function GuestSpark() { return <View className="mini-page"><View className="card empty-card"><Text className="card-title">请先登录</Text><Text className="muted">登录后才能查看好友火花与任务开关。</Text><Button className="mini-button primary-button" onClick={() => Taro.switchTab({ url: '/pages/login/index' })}>去登录 / 绑定</Button></View></View> }
function EmptySpark() { return <View className="mini-page"><View className="card empty-card"><Text className="card-title">还没有可管理的账号</Text><Text className="muted">请先在 PC 端绑定抖音账号并同步好友。</Text></View></View> }
