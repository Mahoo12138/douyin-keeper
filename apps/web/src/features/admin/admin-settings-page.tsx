...
import { Pencil, Plus, RefreshCw, Save, Settings2, X } from 'lucide-react'
...
export function AdminSettingsPage() {
	...
	return <div className="space-y-8"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-medium text-primary">控制台 · 配置</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">站点设置</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">维护站点模块使用的 JSON 配置。更新动作会记录配置键，但审计日志不会保存具体配置值。</p></div><Button variant="outline" onClick={() => void settingsQ.refetch()} disabled={settingsQ.isFetching}><RefreshCw />重新加载</Button></div>{updateMutation.isError && <Card className="border-destructive/40"><CardContent className="py-4 text-sm text-destructive">设置保存失败，请检查配置键、JSON 格式或管理员权限后重试。</CardContent></Card>}<Card><CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0"><div><CardTitle>{editingKey ? '编辑配置' : '新增配置'}</CardTitle><CardDescription>单个配置值最大 16 KB；敏感凭据不应存入站点设置。</CardDescription></div>{editingKey && <Button variant="ghost" size="icon" onClick={resetForm} aria-label="取消编辑"><X /></Button>}</CardHeader><CardContent><form className="space-y-4" onSubmit={submit}><div className="space-y-1.5"><Label htmlFor="admin-setting-key">配置键</Label><Input id="admin-setting-key" value={key} disabled={!!editingKey} onChange={(event) => setKey(event.target.value)} placeholder="例如 feature.notice" autoComplete="off" /><p className="text-xs text-muted-foreground">只允许小写字母、数字、点、短横线和下划线。</p></div><div className="space-y-1.5"><Label htmlFor="admin-setting-value">JSON 配置值</Label><textarea id="admin-setting-value" value={value} onChange={(event) => setValue(event.target.value)} rows={8} spellCheck={false} className="flex w-full rounded-md border border-input bg-background px-3 py-2 font-mono text-sm shadow-sm outline-none placeholder:text-muted-foreground focus-visible:ring-1 focus-visible:ring-ring" placeholder={'{"enabled":true}'} /></div>{formError && <p className="text-sm text-destructive" role="alert">{formError}</p>}<div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={resetForm} disabled={updateMutation.isPending}>清空</Button><Button type="submit" disabled={updateMutation.isPending}><Save />{updateMutation.isPending ? '保存中…' : '保存配置'}</Button></div></form></CardContent></Card><Card><CardHeader><CardTitle>已保存配置</CardTitle><CardDescription>{settings.length ? `共 ${settings.length} 项；按配置键排序。` : '尚未保存任何站点配置。'}</CardDescription></CardHeader><CardContent>{settingsQ.isPending ? <SettingsLoading /> : settingsQ.isError ? <div className="rounded-lg border border-destructive/30 bg-destructive/5 px-4 py-10 text-center"><p className="font-medium">配置暂时不可用</p><Button className="mt-4" variant="outline" onClick={() => void settingsQ.refetch()}>重试</Button></div> : settings.length ? <div className="space-y-3">{settings.map((setting) => <SettingRow key={setting.key} setting={setting} onEdit={() => editSetting(setting)} />)}</div> : <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12 text-center"><Settings2 className="size-8 text-muted-foreground" /><p className="mt-3 text-sm text-muted-foreground">暂无站点配置</p><Button className="mt-4" variant="outline" size="sm" onClick={() => { setEditingKey(null); document.getElementById('admin-setting-key')?.focus() }}><Plus />添加第一项</Button></div>}</CardContent></Card></div>
}
