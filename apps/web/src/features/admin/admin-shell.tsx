...
export const adminNavGroups: readonly AdminNavGroup[] = [
  { label: '概览', items: adminNav.slice(0, 1) },
  { label: '资源与运营', items: adminNav.slice(1, 7) },
  { label: '系统配置', items: adminNav.slice(7) },
]
...
export function AdminShell({ children }: { children: ReactNode }) {
  ...
  function renderNav() {
    return adminNavGroups.map((group) => <section key={group.label} aria-labelledby={`admin-nav-${group.label}`}>
      <h2 id={`admin-nav-${group.label}`} className="px-3 pb-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">{group.label}</h2>
      <div className="space-y-1">{group.items.map((item) => <Link key={item.to} to={item.to} onClick={closeMobileNav} className={`flex items-center gap-3 rounded-md px-3 py-2 text-sm transition-colors ${active(item.to, item.exact === true) ? 'bg-sidebar-accent font-medium text-sidebar-accent-foreground' : 'text-muted-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground'}`}><item.icon className="size-4" />{item.label}</Link>)}</div>
    </section>)
  }
    ...
    <div className="flex min-h-screen bg-muted/20">
      <aside className="hidden w-64 shrink-0 flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground md:flex">
        ...
        <nav className="flex-1 space-y-6 p-3">
          <div className="space-y-6" aria-label="管理控制台导航">{renderNav()}</div>
        </nav>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b bg-background/90 px-4 backdrop-blur-sm md:px-8">
          <div className="flex items-center gap-3"><Button variant="ghost" size="icon" className="md:hidden" aria-label={mobileNavOpen ? '关闭管理导航' : '打开管理导航'} aria-expanded={mobileNavOpen} aria-controls="admin-mobile-nav" onClick={() => setMobileNavOpen((open) => !open)}>{mobileNavOpen ? <X /> : <Menu />}</Button><Link to="/dashboard" className="font-semibold md:hidden">Douyin Keeper</Link><span className="hidden text-sm text-muted-foreground md:inline">管理控制台</span></div>
        ...
        {mobileNavOpen && <div id="admin-mobile-nav" className="fixed inset-0 z-50 md:hidden"><button type="button" aria-label="关闭管理导航" className="absolute inset-0 bg-black/40" onClick={closeMobileNav} /><aside className="relative flex h-full w-72 max-w-[85vw] flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground shadow-xl"><div className="flex h-16 items-center justify-between border-b border-sidebar-border px-5"><div className="flex items-center gap-3"><div className="flex size-8 items-center justify-center rounded-lg bg-sidebar-primary text-sidebar-primary-foreground">火</div><div><div className="font-semibold">Douyin Keeper</div><div className="text-xs text-muted-foreground">Admin Console</div></div></div><Button variant="ghost" size="icon" aria-label="关闭管理导航" onClick={closeMobileNav}><X /></Button></div><nav className="flex-1 space-y-6 overflow-y-auto p-3" aria-label="管理控制台导航"><div className="space-y-6">{renderNav()}</div></nav></aside></div>}
