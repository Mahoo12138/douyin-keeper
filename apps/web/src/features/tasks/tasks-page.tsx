...
export function TasksPage() {
  ...
  const selectableConversationsByAccount = useMemo(() => {
    const result = new Map<string, Friend[]>()
    friendsByAccount.forEach((items, accountId) => result.set(accountId, selectableTaskConversations(items)))
    return result
  }, [friendsByAccount])
  ...
  function openCreate() {
    ...
    setEditor({ mode: 'create', draft: { accountId: account.id, friendId: conversation.id, enabled: true, timezone: 'Asia/Shanghai', windowStart: '19:30', windowEnd: '22:30', messageKind: 'text', message: '', allowFirstMessage: false } })
  }
  ...
  function changeEditorAccount(accountId: string) {
    ...
    setEditor((current) => current ? { ...current, draft: { ...current.draft, accountId, friendId: conversation?.id ?? '' } } : current)
  }
  ...
  const editorConversations = editor
    ? selectableTaskConversations(
        friendsByAccount.get(editor.draft.accountId) ?? [],
        editor.mode === 'edit' ? editor.draft.friendId : undefined,
      )
    : []
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <Button onClick={openCreate} disabled={friendsLoading}><Plus />{friendsLoading ? '加载会话中…' : '新建任务'}</Button>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {pageState === 'tasks-error' ? <TaskDataError title="任务列表暂时不可用" description="请重试加载任务；现有账号数据不会受到影响。" onRetry={() => void tasksQ.refetch()} /> : visibleTasks.length ? <TaskTable tasks={visibleTasks} accounts={accounts} friends={friends} busyTaskId={busyTaskId} onToggle={(task, enabled) => void toggleTask(task, enabled)} onEdit={openEdit} onRun={setRunTarget} onDelete={setDeleteTarget} /> : <div className="flex flex-col items-center justify-center rounded-xl border border-dashed py-12 text-center"><p className="font-medium">{tasks.length ? '没有符合条件的任务' : '还没有火花任务'}</p><p className="mt-1 text-sm text-muted-foreground">{tasks.length ? '尝试清除筛选条件。' : '为已开启火花维护的会话创建第一个每日任务。'}</p>{!tasks.length && <Button className="mt-4" variant="outline" onClick={openCreate}>创建任务</Button>}</div>}
