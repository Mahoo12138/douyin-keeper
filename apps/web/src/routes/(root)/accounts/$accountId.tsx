...
function FriendsTab({ friends, tasks, accountId, loading, error, pendingFriendId, onToggle, onRetry, hasNextPage, fetchingNextPage, onLoadMore }: { friends: Friend[]; tasks: SparkTask[]; accountId: string; loading: boolean; error: boolean; pendingFriendId: string | null; onToggle: (friend: Friend, enabled: boolean) => void; onRetry: () => void; hasNextPage: boolean; fetchingNextPage: boolean; onLoadMore: () => void }) {
	return <Card><CardHeader><CardTitle className="flex items-center gap-2"><UsersRound className="size-4" />账号好友</CardTitle><CardDescription>{friends.length ? `已加载 ${friends.length} 位好友；可直接调整火花维护开关。` : '好友同步后会出现在这里。'}</CardDescription></CardHeader><CardContent>{loading ? <DetailListLoading /> : error ? <DetailError text="好友数据暂时不可用" onRetry={onRetry} /> : friends.length ? <><div className="overflow-x-auto"><FriendTable friends={friends} tasks={tasks} accountId={accountId} pendingFriendId={pendingFriendId} onToggle={onToggle} /></div>{hasNextPage && <div className="mt-4 flex justify-center"><Button variant="outline" onClick={onLoadMore} disabled={fetchingNextPage}>{fetchingNextPage ? '加载中…' : '加载更多好友'}</Button></div>}</> : <EmptyPanel icon={<UsersRound />} text="还没有同步好友" />}</CardContent></Card>
}
