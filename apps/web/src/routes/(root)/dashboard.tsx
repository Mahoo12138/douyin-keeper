...
function DashboardPage() {
	...
	return (
		<div className="space-y-6">
			...
			<div className="grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(19rem,0.6fr)]">
				<Card>
				...
				<Card>
					...
					<CardContent>{notificationsQ.isLoading ? <Skeleton className="h-24 w-full" /> : notificationsQ.isError ? <ErrorLine text="通知数据暂时不可用" onRetry={() => void notificationsQ.refetch()} /> : notifications.length ? <div className="divide-y">{notifications.slice(0, 3).map((item) => <div key={item.id} className="flex items-start gap-3 py-4 first:pt-0 last:pb-0"><span className={`mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-lg ${item.priority === 'critical' ? 'bg-destructive/10 text-destructive' : 'bg-amber-500/10 text-amber-600 dark:text-amber-400'}`}><ShieldAlert className="size-4" /></span><div className="min-w-0"><p className="text-sm font-medium">{item.title}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{userFacingNotificationBody(item.body)}</p></div></div>)}</div> : <EmptyLine icon={<CheckCircle2 />} text="暂无风险通知" action="打开通知" to="/notifications" />}</CardContent>
				</Card>
			</div>

			<Card>
				<CardHeader><CardTitle>快速开始</CardTitle><CardDescription>还没有完成配置？从这里继续。</CardDescription></CardHeader>
				<CardContent className="grid gap-3 sm:grid-cols-3"><QuickAction icon={<Smartphone />} title="绑定抖音账号" description="扫码或短信绑定" to="/accounts" /><QuickAction icon={<UserRound />} title="同步会话" description="从消息面板读取好友与群聊" to="/conversations" /><QuickAction icon={<ListChecks />} title="配置火花任务" description={entQ.data?.active ? '设置每日执行窗口' : '先兑换权益再配置'} to={entQ.data?.active ? '/tasks' : '/entitlement'} /></CardContent>
			</Card>
		</div>
	)
}
...
function AccountSummary({ account, taskCount, sends }: { account: Account; taskCount: number; sends: { pending: number; succeeded: number; failed: number; next?: { friend: { display_name: string }; scheduled_at: string } } }) {
	return <div className="py-5 first:pt-0 last:pb-0"><div className="flex items-start justify-between gap-4"><div className="flex min-w-0 items-start gap-3"><Avatar className="size-10"><AvatarImage src={account.avatar_url ?? undefined} alt={`${account.nickname || '抖音账号'}头像`} /><AvatarFallback>{(account.nickname || '抖').slice(0, 1)}</AvatarFallback></Avatar><div className="min-w-0"><p className="truncate font-medium">{account.nickname || '未命名账号'}</p><div className="mt-1.5 flex flex-wrap gap-1.5"><StatusBadge label={bindingLabel(account.binding_status)} variant={account.binding_status === 'bound' ? 'success' : 'muted'} /><StatusBadge label={sessionLabel(account.session_status)} variant={account.session_status === 'valid' ? 'success' : account.session_status === 'expired' ? 'destructive' : 'warning'} /><StatusBadge label={riskLabel(account.risk_status)} variant={account.risk_status === 'normal' ? 'success' : account.risk_status === 'paused' ? 'destructive' : 'warning'} /></div></div></div><Button asChild variant="ghost" size="sm" className="shrink-0"><Link to="/accounts">进入账号<ArrowRight /></Link></Button></div><div className="mt-4 grid gap-2 text-xs text-muted-foreground sm:grid-cols-2"><p>最近会话同步：{formatDate(account.last_friend_sync_at)}</p><p className="truncate sm:text-right">下一次：{sends.next ? `${sends.next.friend.display_name} · ${formatDate(sends.next.scheduled_at)}` : '暂无计划'}</p></div><div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 border-t pt-4 sm:grid-cols-4"><Metric label="启用任务" value={taskCount} /><Metric label="今日发送" value={sends.succeeded} /><Metric label="待处理" value={sends.pending} /><Metric label="失败" value={sends.failed} /></div></div>
}
