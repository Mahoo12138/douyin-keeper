...
import { createAdminUserEntitlementGrant, getAdminUser, getAdminUserEntitlements, listAdminEntitlementPlans, revokeAdminEntitlementGrant, updateAdminUser } from '@douyin-keeper/sdk-ts'
...
function AdminUserEntitlements() {
  ...
  const error = userQ.error ?? entitlementsQ.error ?? plansQ.error ?? grantMutation.error ?? revokeMutation.error ?? statusMutation.error

  function toggleUserStatus() {
    if (!user) return
    const nextStatus = user.status === 'active' ? 'disabled' : 'active'
    const prompt = nextStatus === 'disabled' ? '禁用后该用户的全部 Web/Admin/小程序会话会立即失效，确认继续？' : '恢复后不会自动恢复旧会话，用户需要重新登录，确认继续？'
    if (window.confirm(prompt)) statusMutation.mutate(nextStatus)
  }

  return <div className="space-y-8"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><Link to="/admin/users" className="text-sm text-muted-foreground hover:text-foreground">← 返回用户管理</Link><p className="mt-4 text-sm font-medium text-primary">控制台 · 用户授权</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">{user?.display_name || '用户授权'}</h1><p className="mt-2 text-sm text-muted-foreground">查看用户状态与授权时间线，管理权益和访问权限。</p></div><Button variant="outline" onClick={() => { void userQ.refetch(); void entitlementsQ.refetch(); void plansQ.refetch() }} disabled={userQ.isFetching || entitlementsQ.isFetching || plansQ.isFetching}>重新加载</Button></div>{error && <Card className="border-destructive/40"><CardContent className="py-4 text-sm text-destructive">{error instanceof Error ? error.message : '用户数据暂时不可用，请稍后重试。'}</CardContent></Card>}{userQ.isPending || entitlementsQ.isPending || plansQ.isPending ? <DetailLoading /> : user ? <><Card><CardHeader className="flex flex-row items-start justify-between gap-4"><CardTitle className="text-base">用户信息</CardTitle><Button variant={user.status === 'active' ? 'destructive' : 'outline'} onClick={toggleUserStatus} disabled={statusMutation.isPending}>{statusMutation.isPending ? '处理中…' : user.status === 'active' ? '禁用用户' : '恢复用户'}</Button></CardHeader><CardContent className="grid gap-4 text-sm sm:grid-cols-3"><div><div className="text-muted-foreground">用户 ID</div><div className="mt-1 truncate font-mono text-xs" title={user.id}>{user.id}</div></div><div><div className="text-muted-foreground">角色 / 状态</div><div className="mt-1">{user.role === 'admin' ? '管理员' : '用户'} · {user.status === 'active' ? '正常' : '已停用'}</div></div><div><div className="text-muted-foreground">最近登录</div><div className="mt-1">{formatDate(user.last_login_at)}</div></div><div><div className="text-muted-foreground">账号 / 任务</div><div className="mt-1">{user.account_count} / {user.task_count}</div></div><div><div className="text-muted-foreground">权益到期</div><div className="mt-1">{formatDate(user.entitlement_expires_at)}</div></div><div><div className="text-muted-foreground">授权记录</div><div className="mt-1">{grants.length} 条{entitlementsQ.hasNextPage ? '（已加载部分）' : ''}</div></div></CardContent></Card><section className="space-y-3"><div><h2 className="text-lg font-semibold">人工授权</h2><p className="text-sm text-muted-foreground">新授权会按现有未撤销授权的到期时间顺延；已停用用户不能接收新授权。</p></div><Card><CardContent className="p-4"><AdminGrantCreateForm plans={plans} pending={grantMutation.isPending} disabled={user.status !== 'active'} onSubmit={(value) => grantMutation.mutate(value)} /></CardContent></Card></section><section className="space-y-3"><div><h2 className="text-lg font-semibold">授权时间线</h2><p className="text-sm text-muted-foreground">撤销只改变授权状态，保留原始记录用于审计。</p></div>{grants.length ? <><AdminUserGrantTable grants={grants} pendingGrantId={revokeMutation.isPending ? revokeMutation.variables?.grantId : undefined} onRevoke={(grantId, reason) => revokeMutation.mutate({ grantId, reason })} />{entitlementsQ.hasNextPage && <div className="flex justify-center"><Button variant="outline" onClick={() => void entitlementsQ.fetchNextPage()} disabled={entitlementsQ.isFetchingNextPage}>{entitlementsQ.isFetchingNextPage ? '加载中…' : '加载更多授权记录'}</Button></div>}</> : <Card><CardContent className="py-12 text-center text-sm text-muted-foreground">暂无授权记录。</CardContent></Card>}</section></> : <Card><CardContent className="py-12 text-center text-sm text-muted-foreground">用户不存在。</CardContent></Card>}</div>
}
