...
import { listAdminAccounts } from '@douyin-keeper/sdk-ts'
...
function AdminAccounts() {
  ...
  return <div className="space-y-8"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-medium text-primary">控制台 · 资源</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">抖音账号</h1><p className="mt-2 text-sm text-muted-foreground">查看账号归属、登录状态、风险、能力快照和今日发送情况。</p></div><Button variant="outline" onClick={() => void accountsQ.refetch()} disabled={accountsQ.isFetching}>重新加载</Button></div>{accountsQ.isPending ? <AccountTableLoading /> : accountsQ.isError ? <Card><CardHeader><CardTitle>账号数据暂时不可用</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground">请检查管理员权限或稍后重试。</p><Button className="mt-4" variant="outline" onClick={() => void accountsQ.refetch()}>重试</Button></CardContent></Card> : accounts.length ? <AdminAccountTable accounts={accounts} /> : <Card><CardContent className="py-14 text-center text-sm text-muted-foreground">暂无抖音账号记录。</CardContent></Card>}</div>
}
