  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <p className="text-sm font-medium text-primary">控制台 · 权益</p>
          <h1 className="mt-1 text-3xl font-semibold tracking-tight">权益与卡密</h1>
          <p className="mt-2 text-sm text-muted-foreground">管理授权方案、一次性卡密批次和兑换事实；数据库只保存卡密摘要。</p>
        </div>
        <Button variant="outline" onClick={() => void reload()} disabled={plansQ.isFetching || batchesQ.isFetching || redemptionsQ.isFetching}>
          重新加载
        </Button>
      </div>
      {error && (
        <Card className="border-destructive/40">
          <CardContent className="py-4 text-sm text-destructive">{error instanceof Error ? error.message : '权益数据暂时不可用，请稍后重试。'}</CardContent>
        </Card>
      )}
      <section className="space-y-3">
        ...
        {plansQ.isPending ? (
          ...
          <>
            <AdminPlanTable plans={plans} pendingPlanId={disablePlanMutation.isPending ? disablePlanMutation.variables : undefined} onDisable={(id) => disablePlanMutation.mutate(id)} />
            {plansQ.hasNextPage && (
              <div className="flex justify-center">
                <Button variant="outline" onClick={() => void plansQ.fetchNextPage()} disabled={plansQ.isFetchingNextPage}>
                  {plansQ.isFetchingNextPage ? '加载中…' : '加载更多方案'}
                </Button>
              </div>
            )}
          </>
        ) : (
          <EmptyState text="暂无权益方案。" />
        )}
      </section>
      <section className="space-y-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        {generatedCodes.length > 0 && (
          <Card className="border-primary/30">
            <CardHeader>
              <CardTitle className="text-base">本次生成的卡密（仅当前页面会话可见）</CardTitle>
            </CardHeader>
            <CardContent>
              ...
              <div className="mt-3 flex gap-2">
                <Button size="sm" onClick={downloadCodes}>下载 CSV</Button>
                <Button size="sm" variant="ghost" onClick={() => setGeneratedCodes([])}>清除明文</Button>
              </div>
            </CardContent>
          </Card>
        )}
        {batchesQ.isPending ? (
          ...
          <>
            <AdminBatchTable batches={batches} pendingBatchId={disableBatchMutation.isPending ? disableBatchMutation.variables : undefined} onDisable={(id) => disableBatchMutation.mutate(id)} />
            {batchesQ.hasNextPage && (
              <div className="flex justify-center">
                <Button variant="outline" onClick={() => void batchesQ.fetchNextPage()} disabled={batchesQ.isFetchingNextPage}>
                  {batchesQ.isFetchingNextPage ? '加载中…' : '加载更多批次'}
                </Button>
              </div>
            )}
          </>
        ) : (
          <EmptyState text="暂无卡密批次。" />
        )}
      </section>
      <section className="space-y-3">
        <div>
          <h2 className="text-lg font-semibold">兑换记录</h2>
          <p className="text-sm text-muted-foreground">只展示卡密指纹和授权状态，不展示完整卡密。</p>
        </div>
        {redemptionsQ.isPending ? (
          <ListLoading />
        ) : redemptions.length ? (
          <>
            <AdminRedemptionTable redemptions={redemptions} />
            {redemptionsQ.hasNextPage && (
              <div className="flex justify-center">
                <Button variant="outline" onClick={() => void redemptionsQ.fetchNextPage()} disabled={redemptionsQ.isFetchingNextPage}>
                  {redemptionsQ.isFetchingNextPage ? '加载中…' : '加载更多兑换记录'}
                </Button>
              </div>
            )}
          </>
        ) : (
          <EmptyState text="暂无兑换记录。" />
        )}
      </section>
    </div>
  )
