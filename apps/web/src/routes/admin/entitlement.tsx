...
function AdminEntitlement() {
  ...
  const batchesQ = useQuery({ queryKey: ['admin-card-batches'], queryFn: () => listAdminCardBatches(token as string, { limit: 100 }), enabled: !!token })
  const redemptionsQ = useQuery({ queryKey: ['admin-redemptions'], queryFn: () => listAdminRedemptions(token as string, { limit: 100 }), enabled: !!token })
  ...
  const redemptions = redemptionsQ.data?.items ?? []
