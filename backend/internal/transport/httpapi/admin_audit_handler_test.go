...
func TestAdminAuditFilterDecodesCursorAndPreservesFilters(t *testing.T) {
	...
	filter, err := adminAuditFilter(httptest.NewRequest("GET", "/?action=adapter.disable&resource_type=adapter&actor=admin&limit=20&cursor="+encodeAdminAuditCursor(createdAt, 42), nil))
	if err != nil {
		t.Fatal(err)
	}
	if filter.Action != "adapter.disable" || filter.ResourceType != "adapter" || filter.Actor != "admin" || filter.Limit != 20 || filter.AfterID != 42 || filter.AfterCreatedAt == nil || !filter.AfterCreatedAt.Equal(createdAt) {
