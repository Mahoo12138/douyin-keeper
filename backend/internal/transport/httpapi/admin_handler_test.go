...
func TestAdminRuntimeViewIncludesPoolsQueuesAndHealth(t *testing.T) {
	...
	oldestDeadAt := observedAt.Add(-time.Hour)
	view := adminRuntimeViewFrom(admin.RuntimeSummary{
		...
		OutboxPending: 2, OutboxPublishing: 1, OutboxDead: 4,
		OutboxOldestDeadAt: &oldestDeadAt,
	...
	if view.ObservedAt != "2026-08-24T09:00:00Z" || view.RunningJobs != 2 || view.FailedJobs24h != 4 || view.OutboxDead != 4 || !view.SchedulerOnline {
	...
	if view.OutboxOldestDeadAt == nil || *view.OutboxOldestDeadAt != "2026-08-24T08:00:00Z" {
		t.Fatalf("outbox oldest dead time = %v", view.OutboxOldestDeadAt)
	}
}
