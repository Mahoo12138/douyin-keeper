...
Vite 使用 TanStack Router `autoCodeSplitting` 按路由生成异步页面 chunk，并按 React、Router、Query、UI、表单和图标拆分共享 vendor；Web 与 Admin 仍由同一 SPA 发布，但不会把所有页面代码塞入首屏。
...
独立 `conversations.list` Sidecar 已接入消费者会话页 selector、稳定会话/对端 ID 提取和基于平台
会话 ID 的分页游标；selector 不可用或身份字段不稳定时保持 fail-closed。真实抖音账号环境仍需验证
分页抓取的页面结构与返回数据，不在本地用展示名称猜测目标。好友页已支持已解析好友的批量火花开关和已有任务时间窗口
批量编辑，时间窗口沿用任务 PATCH 契约，不隐式创建任务；好友列表已接入统一 cursor pagination，
Web 端通过“加载更多”读取后续页面。会话同步已补齐后端 Job/Outbox/Browser Worker 闭环，按平台会话 ID
分页并在完成后事务性写入会话索引；真实账号的 selector、分页稳定性和返回时间字段仍需环境验证。
...
Web `/dashboard` 对任务和当日发送统计自动补齐所有 cursor 页面，避免总览只计算默认首屏，并与任务页共享统一的无限查询缓存形态。
...
账号详情的任务与发送记录分区也复用统一无限分页查询，支持继续加载，不与列表页使用不同的缓存数据形态；概览发送统计单独按 `Asia/Shanghai` 当日范围查询并自动读取当天全部 cursor 页面。
...
已冻结并保持 fail-closed，Browser adapter 已接入目标会话菜单、身份校验和平台状态/成功提示
回执；平台归档请求的 Job/Outbox、Browser Worker、账号锁与确认回执校验已接线，真实 selector
和平台回执仍待适配器联调。
